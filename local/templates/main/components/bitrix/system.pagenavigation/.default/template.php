...
if ($arResult["NavPageNomer"] > 1) {
    ...
    $navPrev = $uri->addParams(['PAGEN_' . $arResult["NavNum"] => $arResult["NavPageNomer"] - 1])->getUri();
}

// Next
if ($arResult["NavPageCount"] > $arResult["NavPageNomer"]) {
    $uri->deleteParams(['PAGEN_' . $arResult["NavNum"]]);
    $navNext = $uri->addParams(['PAGEN_' . $arResult["NavNum"] => $arResult["NavPageNomer"] + 1])->getUri();
}
...
for ($i = $arResult["nStartPage"]; $i <= $arResult["nEndPage"]; $i++) {
    $pages[] = [
        'num' => $i,
        'isSet' => $i == $arResult["NavPageNomer"],
        'url' => $uri->addParams(['PAGEN_' . $arResult["NavNum"] => $i])->getUri()
    ];
}

?>

    <nav class="pagination-nivagation d-none d-xl-block" aria-label="pagination">
        <ul class="pagination d-flex flex-wrap align-items-center justify-content-center">
            ...
            if (!empty($navPrev)): ?>
                <li class="page-item page-prev-item">
                    <a class="page-link" href="<?= $navPrev ?>" aria-label="Previous"><i class="icon-angle-left"></i></a>
                </li>
            <?
            endif; ?>
            ...
            foreach ($pages as $page): ?>
                <li class="page-item <?= ($page['isSet']) ? 'active' : '' ?>">
            ...
            <li class="page-item">
                <a class="page-link" href="#link">32</a>
            </li>
*/ ?>
            <?
            if (!empty($navNext)): ?>
                <li class="page-item page-next-item">
                    <a class="page-link" href="<?= $navNext ?>" aria-label="Next"><i class="icon-angle-right"></i></a>
                </li>
            <?
            endif; ?>
        </ul>
